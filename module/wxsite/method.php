  ...
  //       $where = empty($where)?'   and  SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradius`*0.01094) ': $where.' and SQRT((`lat` -'.$lat.') * (`lat` -'.$lat.' ) + (`lng` -'.$lng.' ) * (`lng` -'.$lng.' )) < (`pradius`*0.01094) ';
	 ...
	 /*  $datas = json_encode($data);
	  ...
	  function indexshoplistdata(){		// 首页获取附近商家列表（外卖和超市）
		...
		$tempdd = array();
		$tempdd[] =   $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 and is_recom = 1 and endtime > ".time()."  ".$where."    order by ".$orderarray[0]." limit ".$pageinfo->startnum().", ".$pageinfo->getsize()."  ");		
		$tempdd[] =   $this->mysql->getarr("select * from ".Mysite::$app->config['tablepre']."shop where is_pass = 1  and is_open = 1 and is_recom != 1 and endtime > ".time()."  ".$where."    order by ".$orderarray[0]." limit ".$pageinfo->startnum().", ".$pageinfo->getsize()."  ");		
